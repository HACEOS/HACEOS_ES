...
%   Nota: este archivo es libre de tildes para evitar posibles conflictos en sistemas sin codificacion UTF-8
    ...
    methods
        ...
        function obj = extraer_info_estacionario(obj, caso, mr)
            ...
            % Llamar el metodo detectar_elementos_energizados_estacionario
            [id_elementos, id_marcas] = mr.detectar_elementos_energizados_estacionario(obj.circuito_cero_menos);
            
            % Llamar metodo detectar_islas
            obj = mr.detectar_islas(obj.circuito_cero_menos, obj);
                        ...
                        for E = 1:numel(exc)
                            if ~isempty(obj.islas.estacionario.(isla{Y}).(exc{E}).puertas)
                                ...
                                for x = 1:size(obj.islas.estacionario.(isla{Y}).(exc{E}),2)
                                    for p = 1:size(obj.islas.estacionario.(isla{Y})(x).(exc{E}).puertas,1)         
                                        switch obj.islas.estacionario.(isla{Y})(x).(exc{E}).puertas(p,NAT_P)
                                            case {R,L,C}
                                                obj.clases.estacionario.(isla{Y})(x).(exc{E}).puertas{p,1} = @md_puerta_rlc;
                                            case {VI,II}
                                                obj.clases.estacionario.(isla{Y})(x).(exc{E}).puertas{p,1} = @md_puerta_fte_indep;
                                            case {VD,ID}
                                                obj.clases.estacionario.(isla{Y})(x).(exc{E}).puertas{p,1} = @md_puerta_fte_dep;
                                            case LM
                                                obj.clases.estacionario.(isla{Y})(x).(exc{E}).puertas{p,1} = @md_puerta_rlc;
                                        end
                                    end
                                    if ~isempty(obj.islas.estacionario.(isla{Y})(x).(exc{E}).marcas_pol)
                                        for i = 1:size(obj.islas.estacionario.(isla{Y})(x).(exc{E}).marcas_pol,1)
                                            obj.clases.estacionario.(isla{Y})(x).(exc{E}).marcas_pol{i,1} = @hcs.marcas_polaridad;
                                        end
                                    end
                                end
                            end
